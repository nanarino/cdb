...
def music_listen(request):
    if request.method == "GET":
        ...
        return render(request,"music_listen.html",{"audioId":audio_id,"userId":user_id,})
...
def picture_watch(request):
    if request.method == "GET":
        ...
        return render(request, "picture_watch.html", {"album":album,"userId":user_id,"comment_list":comment_list})
...
def library_read(request):
    if request.method == "GET":
        ...
        return render(request, "library_read.html", {"article":article,"userId":user_id,"comment_list":comment_list})
    ...
    if request.method == "POST":
        if request.user.is_authenticated():
            ...
            if new_username!=request.user.username:#用户名被修改
                if models.UserInfo.objects.filter(username=new_username):#新用户名被占用
